...
Matrix createLocalK(int e,mesh &m){
    ...
    productRealMatrix(J*lambda/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixE);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixE,matrixL,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float p_x = m.getParameter(PSI_X);
    float p_y = m.getParameter(PSI_Y);
